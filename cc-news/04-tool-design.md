# ツール設計とAdvanced Tool Use

> 出典: [Writing Effective Tools for Agents](https://www.anthropic.com/engineering/writing-tools-for-agents), [Introducing Advanced Tool Use](https://www.anthropic.com/engineering/advanced-tool-use), [The "think" Tool](https://www.anthropic.com/engineering/claude-think-tool)

## 概要

LLMエージェント向けの高品質なツール設計は、評価駆動の反復的アプローチを使用します。重要な洞察は、「ツールは決定論的システムと非決定論的エージェント間の契約を反映する新しい種類のソフトウェア」であるということです。
...
エージェントのコンテキストは限られているため、ツールはAPIを直接ミラーするのではなく、コンテキスト消費を削減すべきです。

### 原則2: 名前空間の活用

関連するツールを一貫したプレフィックスでグループ化：
...
- 暗号的なUUIDではなく**セマンティック識別子**を返す
- 「concise」または「detailed」モードで柔軟なレスポンス形式をサポート
...
内部テストでは、Claude最適化ツールがSlackとAsana統合の両方で、人間が書いたバージョンを大幅に上回りました。

---

# Part 2: Advanced Tool Use（高度なツール利用）
...
# Part 3: Think Tool（シンクツール）

## 概要

「think」ツールは、「Claudeが行動前に計画を深く検討し反復」させる機能です。複雑な問題解決時にモデルが立ち止まって考える専用スペースを提供します。
...
### 1. ドメイン固有の例を含む戦略的プロンプティング

```xml
<tool name="think">
  <description>
    複雑な決定の前に、一度立ち止まって考えてください。
    例: ユーザーのリクエストがポリシーに準拠しているか確認する前に
    このツールを使用してください。
  </description>
</tool>
```

### 2. システムプロンプトへの詳細なガイダンス配置

ツール説明そのものより、システムプロンプト内に詳細な指針を含めることが効果的です。
